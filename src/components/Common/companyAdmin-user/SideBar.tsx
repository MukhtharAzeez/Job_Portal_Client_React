      ...
      <div className="w-full py-4 px-2 text-gray-900 bg-white rounded-lg text-left capitalize font-medium shadow-2xl">
        ...
        />
        {
          links.map(function (link: Link) {
            ...
            return (
              <Link to={link.href} key={link.title} onClick={() => { link.title == 'Logout' && localStorage.clear() }} className={`cursor-pointer px-2 ${link.href === href ? 'bg-gray-800' : 'bg-white hover:bg-gray-200 hover:text-gray-700'}  rounded flex`}>
